...
2. Write a clear explanation suited to grade "${gradeLevel}". CRITICAL FORMAT RULE: write it as SEPARATE SHORT LINES, each point on its own line, separated by an actual newline character (\\n) — NEVER write it as one continuous paragraph or run-on text with dashes. Each line should be one short, complete idea (max ~15 words per line). Use only simple, everyday words a child uses in daily talk — avoid difficult or bookish words completely; if a hard word is unavoidable, explain it in one simple phrase right after. If this is a grammar, spelling, or language-rule doubt, give AT LEAST 4-5 example sentences (each on its own line) showing correct usage, with common mistakes shown next to the correct form. For other subjects, give at least 2-3 concrete examples, each on its own line. If the concept is visual or spatial (shapes, fractions, processes, cycles, number lines, etc.), include one simple text-based diagram using arrows or dashes on its own lines to illustrate it. Prioritize the student truly understanding — do not worry about total length, just keep EVERY line short and on its own line.
...
Write a COMPLETELY DIFFERENT, even simpler explanation than before — use a new everyday analogy or a step-by-step breakdown. CRITICAL FORMAT RULE: write it as SEPARATE SHORT LINES, each point on its own line (separated by actual newline characters), never as one continuous paragraph. Use only very simple, everyday words (avoid any bookish or technical terms). Prioritize the student truly understanding over being short — do not worry about length, just keep every line short. Include 2-4 concrete examples, each on its own line (if this is a grammar/language doubt, include several right-vs-wrong example sentences, each on its own line). Write as if explaining patiently to a friend, in ${session.explanationLanguage}. If a simple text-based diagram (arrows, dashes, small sketch) would help, include one on its own lines.
